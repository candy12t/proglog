...
const (
	offWidth uint64 = 4 // offset size. 4 byte
	posWidth uint64 = 8 // postion size. 8 byte
...
func newIndex(f *os.File, c Config) (*index, error) {
	...
	// 一度メモリにマップされたファイルはサイズを変更できないため、マップする前にサイズを変更
...
func (i *index) Close() error {
	...
	// 最後のレコードとファイルを終わりの間に使われていない領域が存在するため、サイズを切り詰める
